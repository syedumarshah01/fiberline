...
function BoxDocumentation({ enclosureId, onChanged, networkRevision = 0, onDeleteEnclosure, onHoverCable, onOpenWorksheet, onOpenQrTag }) {
  ...
  // A spliced IN core has already been consumed by its OUT branch. It must not
  // reappear in either the splice or splitter input pickers as a second branch.
  ...
  // Only available/reserved cores can start a new branch. A core marked
  // spliced has already been consumed by an existing IN → OUT splice.
  ...
  return (
    <div>
      ...
      {availableInCoresForBranching.length + availableOutCoresForBranching.length + splitterPorts.length >= 1 ? (
