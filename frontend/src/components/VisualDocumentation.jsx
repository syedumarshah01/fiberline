    ...
    <div className="vdoc">
      ...
      <div className="vdoc-summary">
        ...
        {STATUS_ORDER.filter((s) =>
          s === "available"
            ? summary.available_cores
            : s === "spliced"
              ? summary.spliced_cores
              : s === "terminated"
                ? summary.terminated_cores
      ...
      >
        {/* Column titles */}
        <text x={LEFT_X} y={TOP_Y - 28} className="vdoc-col-title">
          IN ▸ from upstream
        </text>
        <text x={RIGHT_X} y={TOP_Y - 28} className="vdoc-col-title">
          OUT ▸ to downstream
        </text>
        ...
        />
        <text
          x={layout.box.x + layout.box.w / 2}
          y={layout.box.y + 22}
          textAnchor="middle"
          className="vdoc-box-title"
        >
          {enclosure.code}
        </text>
        <text
        ...
        {(doc.splices || []).map((s) => {
          ...
          return (
            ...
            >
              <title>
                {s.cable_a_code}#{s.core_a_number} ⇄ {s.cable_b_code}#
                {s.core_b_number}
            ...
            <g key={sp.id}>
              ...
              {(sp.ports || []).map((p) => {
                ...
                return (
                  ...
                  >
                    <title>
                      Port {p.port_number} → {p.cable_code || "?"}#
                      {p.core_number}
                    </title>
                  </path>
                );
        ...
        {[...layout.left.cards, ...layout.right.cards].map((card) => (
          <g key={card.group.cable.id}>
            ...
            <text
              x={card.x + CARD_PAD}
              y={card.y + 14}
              className="vdoc-cable-code"
            >
              {card.group.cable.code}
            </text>
            <text
              x={card.x + CARD_W - CARD_PAD}
              y={card.y + 14}
              textAnchor="end"
              className="vdoc-cable-sub"
            >
              {card.cores.length}/{card.group.cable.core_count}
            </text>
            <text
              x={card.x + CARD_PAD}
              y={card.y + HEADER_H + 4}
            ...
            >
              ...
              {card.group.cable.customer_label
                ? ` · ${card.group.cable.customer_label}`
                : ""}
            </text>
            {card.cores.map((core) => {
              ...
              return (
                ...
                >
                  <title>
                    {card.group.cable.code} · core #{core.core_number} ·{" "}
                    {core.status}
                  </title>
                </circle>
              );
        ...
        <g transform={`translate(${LEFT_X}, ${layout.height - 66})`}>
          <text x={0} y={8} className="vdoc-col-title">
            STATUS
          </text>
          ...
          <g transform="translate(0, 40)">
            <line
              ...
              strokeWidth={1.8}
            />
            <text x={24} y={3.5} className="vdoc-legend-text">
              fusion splice
            </text>
            <line
              x1={112}
              ...
              strokeDasharray="4 3"
            />
            <text x={136} y={3.5} className="vdoc-legend-text">
              mechanical
            </text>
            <rect
              x={238}
              ...
              rx={2}
              className="vdoc-splitter"
              transform="rotate(45 243 0)"
            />
            <text x={258} y={3.5} className="vdoc-legend-text">
              splitter
            </text>
            <text x={330} y={3.5} className="vdoc-legend-text">
              dot ring = fiber color code · hover dots/wires for details
            </text>
          </g>
        </g>
      </svg>
