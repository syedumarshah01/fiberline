...
describe('POST /api/cables/:id/insert-enclosure — the halves stay one fiber', () => {
  ...
  test('moves existing downstream endpoint splices onto the new downstream core', async () => {
    freshStore();
    const originalCore = coreOf('f1', 1);
    store.splices.find((splice) => splice.id === 's-nap').core_b_id = originalCore.id;

    const res = await insertMidSpanEnclosure();
    assert.equal(res.status, 201);
    const downstream = store.cables.find((cable) => cable.code === 'CBL-F1-B');
    const downstreamCore = coreOf(downstream.id, 1);
    const endpointSplice = store.splices.find((splice) => splice.id === 's-nap');
    assert.equal(endpointSplice.core_b_id, downstreamCore.id,
      'the old downstream connection must follow the downstream half');
    assert.equal(originalCore.status, 'spliced',
      'the original core stays spliced while the automatic middle pass-through exists');
    assert.equal(downstreamCore.status, 'spliced');
  });
