 ...
 */
function floodBoxDownstream(
  index,
  seeds,
  failureBoxIds,
  { maxNodes = DEFAULT_MAX_NODES, explicitOutputCoreIds = [] } = {},
) {
  const parents = new Map();
  const depths = new Map();
  const visited = new Set();
  ...
  const order = [];
  let truncated = false;
  const explicitOutputKeys = new Set(explicitOutputCoreIds.map(coreKey));
  ...
  const outgoingCore = (key) => {
    ...
    // A splitter output_core_id is authoritative even when an imported cable
    // row has its endpoints reversed. The port itself identifies this as an
    // outgoing fibre; do not make the map depend on the cable's from/to order.
    if (explicitOutputKeys.has(key)) return Boolean(core && cable && connected);
  ...
  const edgesAway = (key) => {
    ...
    for (const edge of index.spliceEdges.get(key) || []) {
      ...
      const normalDownstreamSplice =
        boxId &&
        cable.to_enclosure_id === boxId &&
        nextCable?.from_enclosure_id === boxId;
      const explicitOutputSplice =
        explicitOutputKeys.has(key) &&
        boxId &&
        !failureBoxIds.has(boxId) &&
        (cable.from_enclosure_id === boxId || cable.to_enclosure_id === boxId) &&
        (nextCable?.from_enclosure_id === boxId || nextCable?.to_enclosure_id === boxId);
      if (normalDownstreamSplice || explicitOutputSplice) {
        edges.push(edge);
      }
    }
...
function failureSurfaceSeeds(index, { failureBoxIds = new Set(), failureCableIds = new Set() } = {}) {
  ...
  const splitterOutputCoreIds = new Set();
  ...
  for (const splitter of index.splitters) {
    ...
    for (const port of index.portsBySplitter.get(splitter.id) || []) {
      if (port.output_core_id) {
        ...
        splitterOutputCoreIds.add(port.output_core_id);
      }
    }
  }
  ...
  return {
    ...
    splitterOutputCoreIds: [...splitterOutputCoreIds],
  };
}
...
function analyzeImpact({
  ...
  const boxOutputFlood = boxFailureOnly
    ? floodBoxDownstream(index, surface.seedKeys, failureBoxIds, {
        maxNodes,
        explicitOutputCoreIds: surface.splitterOutputCoreIds,
      })
    : null;
  ...
  // A splitter port explicitly documents its output cable. Keep that cable in
  // the failure report even when the cable endpoints were entered backwards and
  // the endpoint-only fallback could not classify its direction. This is still
  // downstream-only: it applies only to output_core_id rows owned by a splitter
  // inside the failed box, never to the cable feeding the box. A rooted network
  // keeps redundant/light paths live by excluding any output core that remains
  // reachable after the failure.
  const forcedSplitterOutputKeys = surface.splitterOutputCoreIds
    .map(coreKey)
    .filter((key) => !directed || !litKeys.has(key));
  const reportKeys = directed
    ? uniq([...darkKeys(), ...forcedSplitterOutputKeys])
    : uniq([...flood.keys, ...forcedSplitterOutputKeys]);
