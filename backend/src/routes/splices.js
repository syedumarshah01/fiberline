...
router.post('/', validateSpliceData, async (req, res, next) => {
  ...
  try {
    ...
    // An already-spliced IN fibre is a valid branch source: it has arrived from
    // upstream at this enclosure and may fan out to another OUT fibre. The
    // direction check prevents a spliced OUT core from being reused as an IN
    // branch point.
    if (
      !canBranchFromCore(coreA, { cable: coreACable, enclosureId: enclosure_id }) ||
      !['available', 'reserved'].includes(coreB.status)
    ) {
      await trx.rollback();
      return res.status(409).json({
        error: coreA.status === 'spliced' && coreACable?.to_enclosure_id !== enclosure_id
