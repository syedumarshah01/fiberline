...
// plus which cores are still free. The queries live in
// services/boxDocumentation.js, because the splice worksheet reads the same
// box and the two must never disagree.
// ---------------------------------------------------------------------------
router.get('/:id/documentation', async (req, res, next) => {
  try {
    const documentation = await loadBoxDocumentation({ enclosureId: req.params.id });
    if (!documentation) return res.status(404).json({ error: 'Enclosure not found' });
