...
**Red means "there is no light in it".** The report — and the map — paints what the failure actually darkened, so the span that *feeds* the failed element is not painted: a cut cable is red together with everything behind it, but the cable that fed it still carries light up to the break, and the box it came from is still lit. Failing a mid-span joint therefore reddens the joint, the half below it and the drops behind those customers — not the whole route back to the OLT, and not the branches that merely share an upstream box. The one exception is the headend's own box: fail *that* and the light source is gone, so everything goes dark. Direction is what makes this work: light flows from the headend root (`headends.root_enclosure_id`), and without one the app says so and walks both ways from the failure point, which may include the feeding span and lit branches — set the root on the OLT box and the red follows the light.
